...
/*
 * IOMMU API for MTK architected MT6589 M4U implementations
 *
 ...
 *
 * Based on driver/iommu/mtk_iommu.c, driver/iommu/mtk_iommu_v1.c
 */
#include <linux/bug.h>
#include <linux/clk.h>
...
#define MT65XX_MMU_INT_ID_PORT_ID	GENMASK(12, 8)
#define MT65XX_MMU_INT_ID_LARB_ID	GENMASK(14, 13)

#define MT65XX_M4U_TF_PORT(TF)			FIELD_GET(MT65XX_MMU_INT_ID_PORT_ID, TF)
#define MT65XX_M4U_TF_LARB(TF)			(FIELD_GET(MT65XX_MMU_INT_ID_LARB_ID, TF) - 1)

/* MTK generation one iommu HW only support 4K size mapping */
#define MT2701_IOMMU_PAGE_SHIFT			12
#define MT2701_IOMMU_PAGE_SIZE			(1UL << MT2701_IOMMU_PAGE_SHIFT)
#define MT6589_LARB_NR_MAX			4
...
static inline int mt6589_m4u_to_larb(int id)
{
	int i;

	for (i = ARRAY_SIZE(mt6589_m4u_in_larb) - 1; i >= 0; i--)
		if ((id) >= mt6589_m4u_in_larb[i])
			return i;

	return 0;
}

static inline int mt6589_m4u_to_port(int id)
{
	int larb = mt6589_m4u_to_larb(id);

	return id - mt6589_m4u_in_larb[larb];
}
...
static irqreturn_t mtk_iommu_v1_isr(int irq, void *dev_id)
{
	...
	fault_larb = MT65XX_M4U_TF_LARB(regval);
	fault_port = MT65XX_M4U_TF_PORT(regval);
				...
				struct device *dev, bool enable)
{
	...
	for (i = 0; i < fwspec->num_ids; ++i) {
		larbid = mt6589_m4u_to_larb(fwspec->ids[i]);
		portid = mt6589_m4u_to_port(fwspec->ids[i]);
...
static struct iommu_device *mtk_iommu_v1_probe_device(struct device *dev)
{
	...
	larbid = mt6589_m4u_to_larb(fwspec->ids[0]);
	if (larbid >= MT6589_LARB_NR_MAX)
		return ERR_PTR(-EINVAL);

	for (idx = 1; idx < fwspec->num_ids; idx++) {
		larbidx = mt6589_m4u_to_larb(fwspec->ids[idx]);
		if (larbid != larbidx) {
