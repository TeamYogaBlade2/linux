...
struct ek79007 {
	...
	struct regulator_bulk_data supplies[EK79007_NUM_SUPPLIES];
...
static int ek79007_unprepare(struct drm_panel *panel)
{
	...
	regulator_bulk_disable(EK79007_NUM_SUPPLIES, ctx->supplies);
...
static int ek79007_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(EK79007_NUM_SUPPLIES, ctx->supplies);
	if (ret) {
...
static int ek79007_enable(struct drm_panel *panel)
{
	...
	if (dsi_ctx.accum_err) {
		dev_err(ctx->dev, "failed to enable panel: %d\n",
			dsi_ctx.accum_err);
		return dsi_ctx.accum_err;
	}
...
static const struct drm_display_mode ek79007_mode = {
	.clock = (1024 + 60 + 10 + 60) * (600 + 10 + 10 + 50) * 60 / 1000,
	.hdisplay = 1024,
	.hsync_start = 1024 + 60,
...
static int ek79007_probe(struct mipi_dsi_device *dsi)
{
	...
	ret = devm_regulator_bulk_get(dev, EK79007_NUM_SUPPLIES, ctx->supplies);
	if (ret)
