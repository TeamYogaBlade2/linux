...
static void prismrv_soft_reset(struct prismrv_device *pv)
{
	...
	writel(EUR_CR_BIF_CTRL_PAUSE, pv->regs + EUR_CR_BIF_CTRL);
	udelay(10);
	v = readl(pv->regs + EUR_CR_BIF_INT_STAT);
	if (v & EUR_CR_BIF_INT_STAT_FAULT_REQ_MASK) {
		writel(EUR_CR_BIF_CTRL_PAUSE | EUR_CR_BIF_CTRL_CLEAR_FAULT,
		       pv->regs + EUR_CR_BIF_CTRL);
		udelay(10);
		writel(EUR_CR_BIF_CTRL_PAUSE, pv->regs + EUR_CR_BIF_CTRL);
	}

	v = EUR_CR_SOFT_RESET_DPM_RESET |
	    EUR_CR_SOFT_RESET_TA_RESET |
	    EUR_CR_SOFT_RESET_USE_RESET |
	    EUR_CR_SOFT_RESET_ISP_RESET |
	    EUR_CR_SOFT_RESET_ISP2_RESET |
	    EUR_CR_SOFT_RESET_TSP_RESET |
	    EUR_CR_SOFT_RESET_PDS_RESET |
	    EUR_CR_SOFT_RESET_PBE_RESET |
	    EUR_CR_SOFT_RESET_MTE_RESET |
	    EUR_CR_SOFT_RESET_TE_RESET |
	    EUR_CR_SOFT_RESET_TCU_L2_RESET |
	    EUR_CR_SOFT_RESET_UCACHEL2_RESET |
	    EUR_CR_SOFT_RESET_TEX_RESET |
	    EUR_CR_SOFT_RESET_IDXFIFO_RESET |
	    EUR_CR_SOFT_RESET_VDM_RESET |
	    EUR_CR_SOFT_RESET_DCU_L2_RESET |
	    EUR_CR_SOFT_RESET_DCU_L0L1_RESET |
	    EUR_CR_SOFT_RESET_ITR_RESET;
